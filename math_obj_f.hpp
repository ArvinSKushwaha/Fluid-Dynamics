...
public:
    ...
    float operator%(Vec3D n)
    {
        return dot(n);
    }

    Vec3D operator()(float x, float y, float z)
    {
        Vec3D d = Vec3D(x, y, z);
        return d;
    }
    ...
    Vec3D transform(float matrix[3][3])
    {
        float x_ = matrix[0][0] * x + matrix[0][1] * y + matrix[0][2] * z;
        float y_ = matrix[1][0] * x + matrix[1][1] * y + matrix[1][2] * z;
        float z_ = matrix[2][0] * x + matrix[2][1] * y + matrix[2][2] * z;
        return Vec3D(x_, y_, z_);
    }

    Vec3D operator*(float matrix[3][3])
    {
...
public:
    ...
    float operator%(Vec2D n)
    {
        return dot(n);
    }

    Vec2D operator()(float x, float y)
    {
        Vec2D d = Vec2D(x, y);
        return d;
    }
    ...
    Vec2D transform(float matrix[2][2])
    {
        float x_ = matrix[0][0] * x + matrix[0][1] * y;
        float y_ = matrix[1][0] * x + matrix[1][1] * y;
        return Vec2D(x_, y_);
    }

    Vec2D operator*(float matrix[2][2])
    {
        return transform(matrix);
    }
...
public:
    ...
    float *data = (float *)malloc(sizeof(float) * a * b * c);
    ...
    void operator()(uint i, uint j, uint k, float v)
    {
        data[index(i, j, k)] = v;
    }

    ScalarField<a, b, c> operator+(float n)
    {
        ScalarField<a, b, c> N;
    ...
    ScalarField<a, b, c> operator<<(float dataIn[a][b][c])
    {
        for (int i = 0; i < matrixSize[0]; ++i)
        {
            for (int j = 0; j < matrixSize[1]; ++j)
    ...
    ScalarField<a, b, c> operator<<(std::array<std::array<std::array<float, c>, b>, a> dataIn)
    {
        for (int i = 0; i < matrixSize[0]; ++i)
        {
            for (int j = 0; j < matrixSize[1]; ++j)
...
public:
    ...
    VectorField<a, b, c> operator*(float n)
    {
        VectorField<a, b, c> N;
        for (int i = 0; i < matrixSize[0]; ++i)
        {
    ...
    {
        float dXdy, dXdz, dYdx, dYdz, dZdx, dZdy;
...
public:
    ...
    void operator()(uint i, uint j, float v)
    {
        data[index(i, j)] = v;
    }

    ScalarPlane<a, b> operator+(float n)
    {
        ScalarPlane<a, b> N;
        for (int i = 0; i < matrixSize[0]; ++i)
        {
    ...
    ScalarPlane<a, b> operator<<(float dataIn[a][b])
    {
        for (int i = 0; i < matrixSize[0]; ++i)
        {
            for (int j = 0; j < matrixSize[1]; ++j)
    ...
    ScalarPlane<a, b> operator<<(std::array<std::array<float, b>, a> dataIn)
    {
        for (int i = 0; i < matrixSize[0]; ++i)
        {
            for (int j = 0; j < matrixSize[1]; ++j)
...
public:
    ...
    VectorPlane<a, b> operator*(float n)
    {
        VectorPlane<a, b> N;
        for (int i = 0; i < matrixSize[0]; ++i)
        {
